...
### Step 1 — orchestrator intake & classification
...
`orchestrator` asks clarifying questions: Who uses this? What states? What mood/aesthetic? Any constraints?

For NEW_PAGE and REDESIGN this is a `framer` dispatch under the `design` lens — framing is what `framer` is,
and the answer is a spec, not a screen.

### Step 2 — References, before anything is designed
`sourcer` under the `design` lens gathers inspiration BEFORE designing:
...
`designer` designs in 4 layers (not all at once):
...
`designer` chooses tools:
- **Pencil MCP** — precise visual design in .pen files
- **Stitch MCP** — AI-generated screens + variants for exploration
- **Code-first** — direct Tailwind/React (`designer` holds `Write`, `Edit` and `Bash`)
- Can use **multiple tools** and compare approaches

### Step 6 — Implementation
`designer` either:
- **Implements directly** (for COMPONENT, POLISH, small changes) — it is a producing engine, not a planner
- **Hands `builder` a rich reference package** (references, brand tokens, Stitch screens, taste-skill dials, animation specs) when the work is code correctness rather than visual judgement
- **Parallel waves** for big pages (several `designer` dispatches on different sections)
...
### Step 8 — Critique loop
1. Dispatch `reviewer` under the `craft` review lens for external perspective — it holds no `Write` or `Edit`,
   so it cannot fix what it finds, which is what keeps the judgement independent
2. It reviews from user POV + professional designer POV
...
### Step 9 — Quality Gate
1. **WCAG accessibility check** — `reviewer` under the `accessibility` review lens (PASS required)
...
## Engines involved
- **`orchestrator`** — classifies, dispatches, holds the founder boundary
- **`framer`** (lens `design`) — the spec: structure, hierarchy, direction
- **`sourcer`** (lens `design`) — the reference board
- **`designer`** — builds and polishes; the only engine with a perception loop
- **`reviewer`** (lenses `craft`, `accessibility`) — judges the rendered output, never edits it

The playbook for this is [.claude/playbooks/design-pass.yml](../playbooks/design-pass.yml).

## Abort Conditions
- Missing spec (no states defined) → `framer` asks the founder in brainstorm
- Existing component covers this → return existing path, no new work
- WCAG BLOCK → fix before shipping
- `reviewer` CRITICAL findings → fix before shipping
...
- **`designer` has full code authority** — can implement directly or hand off to `builder`
