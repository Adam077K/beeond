...
1. `orchestrator` reads `.claude/memory/DECISIONS.md` for overnight context
2. `orchestrator` reads `CLAUDE.md` for current project state
3. `orchestrator` reads `.claude/memory/CODEBASE-MAP.md` for any active dev work
4. `orchestrator` checks context: if > 50%, recommend `/compact` before starting
5. `orchestrator` proposes:
...
`orchestrator` → [plan for the day]
```

## Notes

- If no CLAUDE.md exists: `orchestrator` asks 3 questions to understand the project
