# design-screen — Beeond Design Operating System Workflow

The ultracode orchestration script for the Beeond design pipeline. One screen in, one
...
Return JSON ONLY:
{
  ...
  "feeling_brief": "2-4 sentences: the richness/confidence/polish to hit, expressed for Beeond",
  "direction": "the DIRECTION for the build: the one memorable element, the primary layout move, and the motion-budget tier (1/2/3) — expressed for Beeond",
...
ORIGINAL in Beeond's own design language. Inspired-by, never traced. Do NOT reproduce a
...
with Beeond's locked brand tokens.
...
expressed as Beeond?" PASS = indistinguishable in CRAFT-LEVEL from the references, in Beeond's own
...
Return a SPECIFIC "here is what's missing to reach the references' craft bar, expressed as Beeond"
...
Return JSON ONLY:
{
  ...
  if (critic && critic.verdict === "PASS") {
    log(`Critic PASS on round ${round}. Craft-parity reached (in Beeond's language).`);
    break;
  }
  ...
  const polish = await agent(
    `You are the design-polisher. Your SOLE job is adding CRAFT DENSITY to the Beeond "${screen}" build
