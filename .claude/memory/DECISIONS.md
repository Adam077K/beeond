...
## 2026-09-03 — SITE SPEC 1.0: the home page as seven scenes; system visuals allowed; the record shown live; co-spec replaces generate-and-react (FOUNDER-decided, twelve items; orchestrator-run)

**Context:** After three rejected visual rounds (eleven mockups 08-26, the one-run build 09-02, eighteen hero renders 09-03) the founder said the attempts failed because he had never clarified exactly what he wanted, and asked for a session to spec the site in plain words with the orchestrator as second brain. The founder had re-sorted the reference corpus into purpose buckets that morning; all 115 images were read and a 14-statement readback of his taste was accepted in full.
**Decisions (founder, 2026-09-03):** (1) the structure is an immersive experience of scenes, not a landing-page section list; (2) system visuals — a dashboard, flows, nodes, cards — are allowed, used sparingly, leaving room for the art (supersedes storyboard §0.3.6 as a hard ban); (3) colour is deferred to the coded site; (4) real shoot as reference, then generate; humans and sky rationed; the company's own work is the artefact stock; every asset carries a meaning; (5) spec first, then code — no static image mockups as a gate; (6) the six concepts of 09-02 were impersonal and unconnected to the company; (7) five system scenes + two art breathers in the recommended order; (8) hero = real sky, marks assemble into a finished piece of work, one hand enters; (9) immersion 4; (10) the record shown as if live, no label — an override, risk logged once; (11) the ask draws a symbolic footprint from the typed URL, a person prepares the real audit; (12) the visitor is described by situation, never segment.
**Orchestrator decisions, strike to reverse:** the record shown is Beeond's own real current-month record; scene 5 is built as four stations; the three Step 4 versions differ on the mark only (dots · glyphs · stipple); the headline stays PROVISIONAL from the storyboard until the founder edits it.
**Rationale:** a written target turns a rejection by eye into a diagnosis by line, which is the only anti-supersession device that has a chance here. Research (three sourced packets) corrected eleven paraphrased laws, found award-grade motion is usually cheap and that scrolljacking and parallax test badly, and established that a label does not cure a misleading image — so honesty moved into the content.
**Reversibility:** reversible — markdown only, tier lite, nothing public, nothing merged. NOT PLAN OF RECORD; no customer signal has been earned; the spec is founder thesis and goes in front of the two warm prospects as a test.
**Owner:** founder, via `orchestrator` (orchestrator-site-spec)
**Affects:** `designer` and `builder` (Step 4 builds against `SITE-SPEC.md`, not against references or the storyboard) · `reviewer-readonly` (the gate measures against spec lines) · the founder (six OPEN slots before a build starts).
**Detail:** `docs/05-marketing/SITE-SPEC.md` · `docs/08-agents_work/handoffs/2026-09-03-site-spec-session-plan.md` · packets `2026-09-03-s1a/b/c-*.md` · session `docs/08-agents_work/sessions/2026-09-03-orchestrator-site-spec.md`
