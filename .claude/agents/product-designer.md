...
```bash
pnpm -F @beeond/web dev &   # start dev server if not running
```
...
git commit -m "design(scan): implement scan results screen with tier score cards (BEEOND--N)"
```
...
```json
{
  ...
  "commits": [
    "design(scan): implement scan results page with per-engine score cards (BEEOND--118)",
    "design(scan): add responsive mobile layout for ScanResultCard (BEEOND--118)"
  ],
  "screenshots": [
