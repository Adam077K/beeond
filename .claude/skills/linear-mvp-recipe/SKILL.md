...
description: "Beeond's locked Linear label vocabulary, sub-ticket creation pattern, and single-synthesis-comment style. Use whenever any CEO or C-suite agent interacts with Linear tickets, creates sub-tickets, or posts comments."
tags: [workflow, beeond-specific, linear, orchestration]
...
```
Beeond — Main (product work)        ← Adam and agents work here
Beeond — War Room (orchestration)   ← Standing Routines post here, separate from product
Beeond — Strategy (board meetings)  ← Board meeting outputs
```
