...
const PROMPTS = {
  ...
  // v5: the founder's mock — the artwork IS the hero. Full-bleed comb that
  // bleeds off the top/right/bottom edges; cells LARGE and mostly EMPTY
  // (channel icons + labels overlay in code); watercolor loose and spilling.
  "hero-bleed": {
    size: "1536x1024",
    prompt: `${STYLE_LOCK} A full-page spread from the founders' field journal, drawn LARGE and loose — the sketch dominates the page. THE RIGHT 58% OF THE FRAME: a grand hand-sketched honeycomb of hexagon cells — each cell about ONE SIXTH of the page height — the cluster so big it RUNS OFF the top edge, the right edge and the bottom edge of the page, outer cells cut mid-wall by the page edges as if the journal page could not contain it. Nine to eleven cells fully visible plus several partial cells at the edges, in a clean regular honeycomb lattice. The linework is loose and energetic: scratchy fineliner ink over visible rough pencil construction lines, double and triple stroke passes, corners overshooting slightly. MOST CELLS ARE COMPLETELY EMPTY — clean bare paper inside every empty cell, ready to be labeled, no shading and no hatching inside them. Exactly three cells carry loose transparent golden honey-yellow watercolor washes that SPILL casually past the ink walls with wet edges, plus one soft amber wash stain floating near the comb, and one thin honey drip running from a lower honey cell's wall. ONE cell near the comb's center-left, fully visible, is flooded with solid warm near-black ink and contains a minimal geometric bee emblem drawn in thin golden outline strokes — hexagonal wings, three-band abdomen, an abstract mark, not a realistic insect. ABSOLUTELY NO BEES anywhere. NO letters, NO words, NO icons — empty cells stay bare. TWO thin dashed ink flight paths with playful loops wander in from the left half toward the comb. THE LEFT 42% OF THE FRAME: almost empty near-white paper — only the dashed paths and one faint amber stain low down, fading to completely clean paper at the far left edge.`,
  },
  // v6.1: the hero drawing — meaning first: scattered channels braid into
  // one coordinated, built presence. NO bees (above the fold), NO lettering
  // (HE mirrors the stage). Portrait, lives in the hero's right whitespace.
  "hero-signal": {
    size: "1024x1536",
    prompt: `${STYLE_LOCK} A tall narrow study from the founders' field journal, drawn small and precise with generous empty paper at every edge. TOP HALF: exactly FIVE tiny loose hand-inked pictograms scattered wide apart, each drawn ONCE and all five clearly different, sketchy and slightly unfinished, drifting at different tilted angles: (1) a magnifying glass, (2) a rounded speech bubble containing three small dots, (3) a closed envelope with a plain flap and nothing else drawn on it, (4) a small page with faint ruled lines, (5) a tiny three-bar rising chart. EACH of the five pictograms trails its own thin faint dashed ink path downward; the five dashed paths gradually bend toward one another, meet, and BRAID over each other into ONE confident dashed line with a single playful loop. BOTTOM THIRD: that single braided line arrives at the open top of ONE large hexagon comb cell drawn PERFECTLY FLAT on the paper in plan view — a flat two-dimensional hexagon outline like a cell seen face-on in a diagram, absolutely NOT a 3D box, NOT isometric, NO perspective, NO visible side walls, NO cast shadow — with confident double-stroked ink walls and a thin inner wall line for comb thickness; the hexagon's inner floor holds a wash of completely matte flat honey-yellow watercolor like dried gouache, slightly darker at the edges, with absolutely NO white shine, NO gloss, NO highlight, NO reflections; two faint pencil-ghost hexagon outlines just beginning beside its lower-right wall. ABSOLUTELY NO BEES anywhere in this drawing. No words, no letters, no numbers. The meaning: many scattered channels become one coordinated, built presence.`,
  },
  // v6.2: the DESIGNED hero (founder: "not basic, more the DESIGN") — same
  // meaning, master-plan draftsman craft: comb under construction + channel
  // call-outs braiding into the heart cell. Still zero bees, zero lettering.
  "hero-signal-v2": {
    size: "1024x1536",
    prompt: `${STYLE_LOCK} A masterfully composed tall page detail from the founders' field journal — a working MASTER PLAN drawing, dense with draftsman's craft yet airy with clean paper. THE LOWER TWO THIRDS: a hand-sketched honeycomb cluster of about seven hexagon cells under construction, arranged at a slight lively diagonal: the outermost cells only faint pencil ghost lines with visible construction guides still uncorrected; the middle cells confident double-stroked ink walls with a thin inner wall line for real comb thickness, one of them washed in one flat pale warm-grey; and THE HEART CELL, slightly larger, flooded with matte luminous honey-yellow watercolor like dried gouache — pigment pooling slightly darker along the wall edges, absolutely NO white shine, NO gloss — with a fine straight ink meniscus line and ONE slow drip of honey running over its lower wall ending in a hanging drop. Around the cluster, the marks of someone DESIGNING a system, not decorating: small engineer's ruler tick-marks along one outer wall, one faint compass-swing arc in pencil, two or three tiny measurement dashes. THE UPPER THIRD: five small channel pictograms — a magnifying glass, a rounded speech bubble with three dots, a plain closed envelope, a small page with faint ruled lines, a tiny three-bar rising chart — each drawn ONCE, each inside its own tiny imperfect hand-drawn rounded frame like a specimen call-out, pinned at five lively different angles; from each call-out a thin dashed ink leader line sweeps downward, and the five dashed lines BRAID into ONE confident dashed line with a single playful loop before arriving at the heart cell's open top. One small strip of aged washi tape at the very top edge, as if the page were pinned into the journal. ABSOLUTELY NO BEES anywhere. No words, no letters, no numbers. The meaning: five scattered channels, designed into one built system.`,
  },
  // v6.1: small margin spot-drawings for the quieter sections (transparent
  // specimen sheet → crop by alpha components).
  "spot-sheet": {
    size: "1024x1024",
    transparent: true,
    prompt: `${STYLE_LOCK} A specimen sheet of FOUR small separate ink-and-wash spot drawings isolated on a COMPLETELY TRANSPARENT background, arranged loosely in a two-by-two grid, generously spaced, none touching: (1) FOUNDATION — three hexagon cells being laid like bricks in a corner, the lowest one solid confident ink, the next only half-outlined, the third just faint pencil ghost lines, with two tiny engineer's ruler tick-marks; (2) ENGINE — a small neat stack of three thin hand-sketched paper sheets with faint ruled lines and no letters, the top sheet lifting at one corner as if just finished, one quick stroke of honey-yellow wash along the lifting edge; (3) AMPLIFY — a small hexagon cell with eight short confident dashed ink rays radiating outward from it in all directions like a signal, one ray ending in a tiny solid yellow watercolor dot; (4) VESSEL — one open hexagon cell with double-stroked walls and two small strips of aged washi tape across its top corners as if a photograph will be pinned inside, its interior completely empty. Each spot drawn quick and imperfect like margin glyphs in a field journal, same scratchy warm near-black ink, watercolor used only where specified. ABSOLUTELY NO BEES. No words, no letters.`,
  },
  "icon-sheet": {
    size: "1024x1024",
    transparent: true,
    prompt: `${STYLE_LOCK} A specimen sheet of SIX tiny hand-inked pictograms isolated on a COMPLETELY TRANSPARENT background, arranged in a loose two-row grid, generously spaced, none touching: (1) a magnifying glass; (2) a pencil writing on a small page; (3) a rounded speech bubble with three dots; (4) a closed envelope; (5) a browser window with a title bar line; (6) a small rising bar chart of three bars. Each pictogram drawn in the same scratchy warm near-black fineliner ink with wobbly confident double strokes, quick and imperfect like margin glyphs in a field journal, all at the same visual weight and roughly the same size. INK ONLY — no color, no washes, no paper, no words, no bees.`,
  },
  "bee-sheet": {
    size: "1024x1024",
    transparent: true,
