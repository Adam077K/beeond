...
|-------------|---------|---------|
| `Beeond` | Display name in agent prompts and brand voice | `Acme` |
| `beeond` | lower-case slug used in env vars / urls | `acme` |
| `BEEOND` | UPPER prefix for env vars (e.g. `ACME_API_KEY`) | `ACME` |
| `BEEOND-` | Linear / Jira ticket prefix | `ACME-` |
| `beeond.com` | Production domain | `acme.com` |
| `https://beeond.com` | Marketing site URL (often a separate platform) | `https://acme.com` |
| `https://github.com/Adam077K/beeond` | GitHub repo URL | `https://github.com/org/repo` |
...
| `.claude/hooks/*.sh` and `*.js` | Hooks run on every action. Read each one before enabling. `gsa-context-monitor.js` references env var `BEEOND_NO_AUTOCOMPACT` — set it or remove the check. |
