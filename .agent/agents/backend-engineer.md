...
# "feat(api): rate-limit free scans (BEEOND--104)"
```
...
```json
{
  ...
  "commits": [
    "feat(api): rate-limit free scans to 5 per IP per hour (BEEOND--104)",
...
|---|---|
| Touching `apps/web/supabase/migrations/` or RLS | `supabase-rls-beeond` |
